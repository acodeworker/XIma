...
#define kOS_IOS10_OR_LATER ( [[[UIDevice currentDevice] systemVersion] floatValue] >= 10.0)
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            else if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleCustomize)
            {
                rect = CGRectMake((self.segmentWidthEx * idx) + (self.segmentWidthEx - stringWidth) / 2, y, stringWidth, stringHeight);
                
                if (kOS_IOS10_OR_LATER)
                {
                    rect = CGRectMake(self.segmentWidthEx * idx, y, self.segmentWidthEx, stringHeight);
                }
                
                rectDiv = CGRectMake((self.segmentWidthEx * idx) - (self.verticalDividerWidth / 2),
                                     self.selectionIndicatorHeight * 2,
                                     self.verticalDividerWidth,
                                     self.frame.size.height - (self.selectionIndicatorHeight * 4));
                
                fullRect = CGRectMake(self.segmentWidthEx * idx, 0, self.segmentWidthEx, oldRect.size.height);
            }
...
//			CGFloat yOffset = roundf(((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2) - (stringHeight / 2));
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                ...
                CGSize size = [self.sectionTitles[idx] boundingRectWithSize:CGSizeMake(MAXFLOAT, 0.0)
                ...
                imageXOffset = (self.segmentWidth * idx) + self.segmentWidth * 0.5f - imageWidth * 0.5f - textWidth * 0.5f - 2.5f;
                
                textXOffset = imageXOffset + imageWidth + 5;
            ...
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - imageHeight) * 0.5f);
            CGRect imageRect = CGRectMake(imageXOffset, imageYOffset, imageWidth, imageHeight);
            CGRect textRect = CGRectMake(textXOffset, imageYOffset, textWidth, stringHeight);
...
- (void)updateSegmentsRects {
    ...
    } else if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleCustomize) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
//            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = self.segmentWidthEx;
        }];
    }else if (self.type == HMSegmentedControlTypeImages) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
            ...
            segment = (touchLocation.x + self.scrollView.contentOffset.x) / self.segmentWidthEx;
        }
...
- (CGFloat)totalSegmentedControlWidth {
    if (self.type == HMSegmentedControlTypeText && self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        return self.sectionTitles.count * self.segmentWidthEx;
    }else {
...
- (void)scrollToSelectedSegmentIndex:(BOOL)animated {
    ...
    if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
        ...
        rectForSelectedIndex = CGRectMake(self.segmentWidthEx * self.selectedSegmentIndex,
                                          0,
                                          self.segmentWidthEx,
                                          self.frame.size.height);
        
        selectedSegmentOffset = (CGRectGetWidth(self.frame) / 2) - (self.segmentWidthEx / 2);
    }
